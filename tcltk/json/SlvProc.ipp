...
static void DoVarAnalyze(Asc_DString *hptr,
...
static void DoRelAnalyze(Asc_DString *hptr,
...
int ascjson::Asc_VarAnalyzeCmd(Asc_DString *hptr,
                    int argc, CONST84 char *argv[])
{
  ...
  if ( g_solvsys_cur == NULL ) {
    ...
    Asc_DStringSet(hptr, "Asc_VarAnalyzeCmd called without slv_system");
    return HELP_ERROR;
  }
  ...
  if (!((low>0)&&(high>0)&&(high<=maxvar))) {
    Asc_DStringSet(hptr, "Invalid index ranges in __var_analyze");
    return HELP_ERROR;
  }
  ...
  if (strncmp(argv[3],"scaling",3)==0) {
    ...
    b_type = b_othervalue;
    if ( argc != 7 ) {
      Asc_DStringAppend(hptr,"A \"other value\" analysis requires an "
    ...
    Asc_DStringSet(hptr, "Invalid analyze type requested\n");
    return HELP_ERROR;
  }
...
int ascjson::Asc_RelAnalyzeCmd(Asc_DString *hptr,
                  int argc, CONST84 char *argv[])
{
  ...
  if ( g_solvsys_cur == NULL ) {
    ...
    Asc_DStringSet(hptr, "Asc_RelAnalyzeCmd called without slv_system");
    return HELP_ERROR;
  }
  ...
  if ( argc < 6 ) {
    Asc_DStringSet(hptr,"wrong # args: Usage :"
  ...
  if (!((low>0)&&(high>0)&&(high<=maxrel))) {
    Asc_DStringSet(hptr, "Invalid index ranges in __rel_analyze");
    return HELP_ERROR;
  }
  ...
  if (strncmp(argv[3],"residual",3)==0) {
    ...
    Asc_DStringSet(hptr, "Invalid rel analyze type requested\n");
    return HELP_ERROR;
  }
  ...
  DoRelAnalyze(hptr,rp,low,high,b_type,tolerance,othervalue,relative_check);
  return HELP_OK;
}


#ifdef cpp_support_monitor
/*
 ...
 * completed properly.
 * If failure, hptr has a message appended.
 */
static
int MonUpdateCheck(struct SlvMonitor *m, Asc_DString *hptr,
                int argc, CONST84 char *argv[], slv_system_t sys)
{
  (void)argc;
  if (MonUpdateData(m,sys)) {
    Asc_DStringAppend2(hptr, argv[0],": malloc failure",(char *)NULL);
    return 1;
  }
  return 0;
}
 ...
 */
static
int MonChange(struct SlvMonitor *m, Asc_DString *hptr,
              int argc, CONST84 char *argv[], slv_system_t sys)
{
  int i,len,do_all=0;
  struct var_variable **vp;
  ...
  real64 *list;
  char buf[40];
  assert(m!=NULL && hptr!=NULL && sys != NULL && argc >= 3);

  if (argc != 4) {
    Asc_DStringAppend6(hptr,"need option to ",argv[0]," ",argv[1]," ",
        argv[2],(char *)NULL);
    return HELP_ERROR;
  }

  if (MonUpdateCheck(m,hptr, argc,argv,sys)) {
    return HELP_ERROR;
  }
  ...
  default:
    ...
    Asc_DStringAppend8(hptr,"unknown option to ",argv[0]," ",argv[1]," ",
 ...
 */
static
int MonGeometry(struct SlvMonitor *m, Asc_DString *hptr,
                int argc, CONST84 char *argv[])
{
  /* parses an int arg n that should look like s and assigns v */
#define CSTI(n,s,v) \
    status = JTcl_GetInt(hptr, argv[(n)],&i); \
    if (status != HELP_OK) { \
       Asc_DStringFree(hptr); \
       Asc_DStringAppend4(hptr,"error parsing ",argv[(n)]," as ",(s), \
           (char *)NULL); \
       return HELP_ERROR; \
    } (v) = i;
...
#define CSTD(n,s,v) \
    status = JTcl_GetDouble(hptr, argv[(n)],&x); \
    if (status != HELP_OK) { \
       Asc_DStringFree(hptr); \
       Asc_DStringAppend4(hptr,"error parsing ",argv[(n)]," as ",(s), \
           (char *)NULL); \
  ...
  assert(m!=NULL);
  if (argc !=10) {
    Asc_DStringAppend6(hptr, argv[0]," ",argv[1]," ", argv[2], " requires "
 ...
 */
static
int MonPlotData (struct SlvMonitor *m, Asc_DString *hptr,
                 int argc, CONST84 char *argv[], slv_system_t sys)
{
  int i,do_all=0;
  struct var_variable **vp;
 ...
 * function for us to hptrret s properly.
 *
 * Most of the internals of this function and downstream use of
 * the data it returns in the hptrreter could be much faster
 ...
 */
int ascjson::SolveMonitor(Asc_DString *hptr, int argc, const char *argv[])
{
#ifndef cpp_support_monitor
  Asc_DStringAppend2(hptr, argv[0],": not ported to webidl yet",(char *)NULL);
  // problem
  return HELP_ERROR;
#else
  slv_system_t sys;
  ...
  if (argc < 3) {
    ...
    if (argv[1][0]=='d') {
      ...
      return Tcl_GlobalEval(hptr,command);
    }
    Asc_DStringAppend(hptr, argv[0],": unknown option ",argv[1],(char *)NULL);
    return HELP_ERROR;
  }
  ...
  case 'c':
    ...
    return MonChange(SMC(cdata),hptr, argc,argv,sys);
  case 'g':
    return MonGeometry(SMC(cdata),hptr, argc,argv);
  case 'p':
    if (sys==NULL) {
      return HELP_OK;
    }
    return MonPlotData(SMC(cdata),hptr, argc,argv,sys);
  default:
    break;
  }
  Asc_DStringAppend(hptr, argv[0],": unknown option",(char *)NULL);
  for (i=1; i<argc; i++) {
    Asc_DStringAppend2(hptr," ",argv[i],(char *)NULL);
  }
  return HELP_ERROR;
#endif
}
 ...
 */
int ascjson::Asc_SolveMonitorCmd(Asc_DString *hptr,
                        int argc, CONST84 char *argv[])
{
  ...
  if (argc!=1) {
    Asc_DStringAppend2(hptr, argv[0],": no arguments allowed yet",(char *)NULL);
    return HELP_ERROR;
  }

  result = SMC(asccalloc(1,sizeof(struct SlvMonitor)));
  if (result==NULL) {
    Asc_DStringAppend2(hptr, argv[0],": insufficient memory",(char *)NULL);
    return HELP_ERROR;
  }
  ...
  if (MonUpdateData(result,g_solvsys_cur)) {
    Asc_DStringAppend3(hptr, argv[0],result->interface_id,
  ...
  // there is no webidl version of this yet; need to switch to returning a string
  // handle and dispatching on a string lookup table in other callbacks.
  // cdata does not exist in the dstring interface
  Asc_AddCommand(hptr, result->interface_id, SolveMonitor,
    (ClientData)result, (Tcl_CmdDeleteProc *)MonDestroy, "solver-monitor",
    SolveMonitorHU, SolveMonitorHS, SolveMonitorHLF);

  Asc_DStringAppend(hptr,result->interface_id,(char *)NULL);
  return HELP_OK;
#endif
}
