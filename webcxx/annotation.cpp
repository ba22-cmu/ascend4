...
list_Annotation AnnotationDatabase::getNotes_list(const Type &type, const SymChar *lang
	, const SymChar *id, const SymChar *method, const int flag
)
{
	return list_Annotation(getNotes(type, lang, id, method, flag));
}
#endif
...
#ifdef __EMSCRIPTEN__
list_Annotation
AnnotationDatabase::getTypeRefinedNotesLang_list(const Type &type
		, const SymChar *lang
)
{
	return list_Annotation(getTypeRefinedNotesLang(type, lang));
}
#endif
/**
