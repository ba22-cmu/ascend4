...
struct Instance *ascjson::g_curinst = NULL;      /* the current instance */


void ascjson::InitInstList(void)
{
  unsigned long c;
...
int ascjson::BrowRootInit(char *sim_name)
{
  struct Instance *ptr;
  if (!sim_name) {
    return 1;
...
int ascjson::Asc_BrowRootInitCmd(Asc_DString *hptr,
                    int argc, CONST84 char *argv[])
{
  ...
  if( argc == 1 ) {
    g_root = g_curinst = NULL;
    g_depth = 0;
    Asc_DStringSet(hptr, "g_instlist initialized\n");
    return HELP_OK;
  } else if ( argc == 2 ) {
    nok = BrowRootInit(QUIET(argv[1]));
    if (nok) {
      Asc_DStringSet(hptr, "simulation not found in \"rootinit\"");
      return HELP_ERROR;
    }
  } else {
    Asc_DStringSet(hptr, "wrong # args: Usage \"rootinit $sim$\"");
    return HELP_ERROR;
  }
  return HELP_OK;
}

int ascjson::Asc_BrowRootCmd(Asc_DString *hptr,
                int argc, CONST84 char *argv[])
{
...
*/
  ...
  if ( argc != 2 ) {
    Asc_DStringSet(hptr, "wrong # args to root");
    return HELP_ERROR;
  }
  if (g_curinst==NULL) {
    Asc_DStringSet(hptr, "Call exp_b $sim$  first!!");
    return HELP_ERROR;
  }
  ...
  if (nch) {
    ...
    if (c) {
      ...
      Asc_DStringSet(hptr, "Child not found - check your root");
      return HELP_ERROR;
    }
  } else {
    Asc_DStringSet(hptr, "At leaves of the Instance Tree");
    return HELP_ERROR;  /* maybe not an error */
  }
}

int ascjson::Asc_BrowRootNCmd(Asc_DString *hptr,
                 int argc, CONST84 char *argv[])
{
...
*/
  ...
  if ( argc != 2 ) {
    Asc_DStringSet(hptr, "wrong # args to \"rootn\"");
    return HELP_ERROR;
  }
  iindex = atol(argv[1]);
  if((iindex >= MAXIMUM_INST_DEPTH) || (iindex < 1)) {
    Asc_DStringSet(hptr, "Invalid args to \"rootn\"");
    return HELP_ERROR;
  }
  ...
  if (iindex < g_depth) {
    ...
    if(i) {
      ...
      Asc_DStringSet(hptr, "Instance for this index, is NULL or not found");
      return HELP_ERROR;
    }
  }
  if (iindex==g_depth) {
  ...
  if (iindex > (g_depth)) {
    Asc_DStringSet(hptr, "Invalid index to \"rootn\" use \"root\" instead");
    return HELP_ERROR;
  }
  return HELP_ERROR;  /* not reached */
}

int ascjson::Asc_BrowRootBackupCmd(Asc_DString *hptr,
                      int argc, CONST84 char *argv[])
{
  ...
  if ( argc != 1 ) {
    Asc_DStringSet(hptr, "wrong # args to oldinst");
    return HELP_ERROR;
  }
  if (g_depth==1) {
    Asc_DStringSet(hptr, "Already at root; Cant backup");
    return HELP_OK;
  }
  g_curinst = g_instlist[g_depth];
  if( ! g_curinst ) {
    Asc_DStringSet(hptr, "Current Instance is NULL; not backing up!!");
    return HELP_ERROR;
  }
  g_depth--;
  ...
  if (newi) {
    ...
    Asc_DStringSet(hptr, "Something is wrong -- previous inst NULL");
    return HELP_ERROR;
  }
}


int ascjson::BrowTransfer(struct gl_list_t *search_list)
{
...
int ascjson::Asc_BrowSimListCmd(Asc_DString *hptr, int argc, CONST84 char *argv[])
{
  ...
  if ( argc != 1 ) {
    Asc_DStringSet(hptr, "wrong # args to \"slist\"");
    return HELP_ERROR;
  }
  sl = g_simulation_list;
  if (sl==NULL) {
    Asc_DStringSet(hptr, "Simulation list is NULL");
  } else {
    len = gl_length(sl);
    for(c=1;c<=len;c++) {
...
int ascjson::Asc_BrowSimTypeCmd(Asc_DString *hptr, int argc, CONST84 char *argv[])
{
  ...
  if ( argc != 2 ) {
    Asc_DStringSet(hptr,"wrong # args: Usage \"simtype\" simname");
    return HELP_ERROR;
  }
  sptr = Asc_FindSimulationRoot(AddSymbol(argv[1]));
  if (sptr) {
    Asc_DStringAppend(hptr,(char *)SCP(InstanceType(sptr)),HALL);
    return HELP_OK;
  }
  Asc_DStringSet(hptr, "Simulation name not found");
  return HELP_ERROR;
}

int ascjson::Asc_BrowInstStatCmd(Asc_DString *hptr,
                    int argc, CONST84 char *argv[])
{

  if ( argc != 1 ) {
    Asc_DStringSet(hptr, "wrong # args to \"bstatistics\"");
    return HELP_ERROR;
  }
  if (g_depth<1) {
    Asc_DStringSet(hptr, "No instances to profile");
    return HELP_OK;
  }
  if (!g_curinst) {
    Asc_DStringSet(hptr, "Null current instance");
    return HELP_ERROR;
  }
...
int ascjson::Asc_BrowInstListCmd(Asc_DString *hptr,
                    int argc, CONST84 char *argv[])
{
  struct Instance *p, *c;
  ...
  if ( argc != 1 ) {
    Asc_DStringSet(hptr, "wrong # args to \"instlist\"");
    return HELP_ERROR;
  }
  if (g_depth<1) {
    Asc_DStringSet(hptr, "No instances to list");
    return HELP_OK;
  }
...
int ascjson::Asc_BrowPrintCmd(Asc_DString *hptr,
                 int argc, CONST84 char *argv[])
{

  if ( argc > 2 ) {
    Asc_DStringSet(hptr, "wrong #args to bprint");
    return HELP_ERROR;
  }
...
int ascjson::Asc_BrowInstIsMutable(struct Instance *i)
{
  if (!i) {
    return 0;                      /* Using 0 - False; 1- True */
  }
...
int ascjson::Asc_BrowInstIsAtomic(struct Instance *i)
{
  if (!i) {
    return 0;                      /* Using 0 - False; 1- True */
  }
...
int ascjson::Asc_BrowInstIsSubAtomic(struct Instance *i)
{
  if (!i) {
    return 0;                      /* Using 0 - False; 1- True */
  }
...
int ascjson::Asc_BrowInstIsConstant(struct Instance *i)
{
  if (!i) {
    return 0;                 /* Using 0 - False; 1- True */
  }
...
static
void BrowWriteUnformattedSet(Asc_DString *hptr,struct Instance *i)
{
  ...
  case string_set:
    ...
    for(c=1;c<=len;c++) {
      if (SetKind(s)==integer_set) {
        sprintf(value,"%lu", FetchIntMember(s,c));
        Asc_DStringAppend2(hptr,value," ",HALL);
      } else {
        Asc_DStringAppend3(hptr,"'",SCP(FetchStrMember(s,c)),"' ", HALL);
      }
    }
    return;
  default:
...
int ascjson::Asc_BrowInstAtomValue(Asc_DString *hptr, struct Instance *i)
{
  enum inst_t kind;
  char value[256];
  char *ustr;
  ...
  case REAL_ATOM_INST:
  case REAL_CONSTANT_INST:
    ustr = Asc_UnitValueDS(i);
    Asc_DStringAppend(hptr,ustr,HALL);
    break;
  case INTEGER_INST:
  case INTEGER_ATOM_INST:
  case INTEGER_CONSTANT_INST:
    sprintf(value,"%ld",GetIntegerAtomValue(i));
    Asc_DStringAppend(hptr,value,HALL);
    break;
  case BOOLEAN_INST:
  case BOOLEAN_ATOM_INST:
  case BOOLEAN_CONSTANT_INST:
  ...
  case SYMBOL_ATOM_INST:
  case SYMBOL_CONSTANT_INST:
    Asc_DStringAppend(hptr,(char *)SCP(GetSymbolAtomValue(i)),HALL);
    break;
  case SET_INST:
  case SET_ATOM_INST:
    BrowWriteUnformattedSet(hptr,i);
    break;
  case REL_INST:
    ustr = Asc_UnitValueDS(i);
    Asc_DStringAppend(hptr,ustr,HALL);
  ...
  case LREL_INST:
    if (GetInstanceLogRel(i)!=NULL) {
      Asc_DStringAppend(hptr,(LogRelResidual(GetInstanceLogRel(i)) ?
                                   "TRUE" : "FALSE"), HALL);
    } else {
      Asc_DStringAppend(hptr,"UNDEFINED",HALL);
    }
    break;
  default:
...
int BrowOperands(Asc_DString *hptr, struct Instance *i)
{
  struct gl_list_t *ol;
  struct Instance *p;
  unsigned long c,len;
  ...
  for (c=1;c <= len; c++) {
    i = (struct Instance *)gl_fetch(ol,c);
    if (i == NULL) {
      continue;
    }
...
int ascjson::Asc_BrowInstQueryCmd(Asc_DString *hptr, int argc, CONST84 char *argv[])
{
  ...
  if (!i) {
    Asc_DStringSet(hptr, "NULL_INSTANCE");
    return HELP_ERROR;
  }
  ...
  if (strncmp(argv[1],"operands",3)==0) {
    return BrowOperands(hptr,i);
  }
  if (strncmp(argv[1],"type",3)==0) {
    Asc_DStringAppend(hptr,SCP(InstanceType(i)),HALL);
    return HELP_OK;
  }
  ...
  if (strncmp(argv[1],"kind",3)==0) {
    ...
    if (result==0) {
      ...
      Asc_DStringAppend(hptr,rstring,HALL);
      ascfree(rstring);
      return HELP_OK;
    } else {
      if (rstring) {
  ...
  if (strncmp(argv[1],"atomvalue",5)==0) {
    if(Asc_BrowInstIsAtomic(i)
      ...
      if (AtomAssigned(i)) {
        result = Asc_BrowInstAtomValue(hptr,i);
      } else {
        Asc_DStringAppend(hptr,"UNDEFINED",HALL);
        return HELP_OK;
      }
  ...
  if (strncmp(argv[1],"child",3)==0) {
    ...
    if (nch) {
      ...
      for(c=1;c<=nch;c++) {
        ...
        case StrName:
          VTcl_AppendElement(hptr,(char *)InstanceNameStr(in));
          break;
        case IntArrayIndex:
  ...
  if (strncmp(argv[1],"parents",3)==0) {
    ...
    if (npa) {
      ...
      for(c=1;c<=npa;c++) {
        ...
        switch(InstanceNameType(in)) {
        case StrName:
          VTcl_AppendElement(hptr,(char *)SCP(InstanceNameStr(in)));
          break;
        case IntArrayIndex:
          sprintf(tmps,"[%ld]",InstanceIntIndex(in));
          VTcl_AppendElement(hptr,tmps);
  ...
  Asc_DStringSet(hptr, "unrecognized command to inst");
  return HELP_ERROR;/*UNREACHED*/
}
...
int ascjson::Asc_BrowInstanceMergeCmd(Asc_DString *hptr, int argc, CONST84 char *argv[])
{
  ...
  if ( argc != 1 ) {
    Asc_DStringSet(hptr, "wrong # args: Usage \"bmerge\"");
    return HELP_ERROR;
  }
  i1 = g_curinst;
  i2 = g_search_inst;
  ...
  case SET_INST: case REL_INST: case LREL_INST:
    Asc_DStringAppend(hptr,"AscendIV does not allow\nmerging "
                     "of \nchildren of Atoms.", HALL);
    return HELP_ERROR;
  default:
    break;
  }
  ...
  if (result==NULL) {
    Asc_DStringAppend(hptr,"Error in merging instances",HALL);
    return HELP_ERROR;
  }
...
int ascjson::Asc_BrowInstanceRefineCmd(Asc_DString *hptr, int argc, CONST84 char *argv[])
{
  ...
  if ( argc != 3 ) {
    Asc_DStringSet(hptr, "wrong # args : Usage \"brefine\" type ?cur?search?");
    return HELP_ERROR;
  }
  if (strncmp(argv[2],"current",3)==0) {
    i = g_curinst;
    ...
    Asc_DStringSet(hptr, "Invalid args to brefine");
    return HELP_ERROR;
  }
  if (!i) {
    Asc_DStringSet(hptr, "Cannot refine a NULL instance");
    return HELP_ERROR;
  }
  switch(InstanceKind(i)) {
  case REAL_INST:
  ...
  if (!desc2) {
    Asc_DStringSet(hptr, "Type not found");
    return  HELP_ERROR;
  }
  if (desc1==desc2) {
    return HELP_OK;
  ...
  if(0 != (desc = MoreRefined(desc1,desc2))) {
    if (desc == desc1) {             /* desc1 more refined than desc2 */
      return HELP_OK;                /* hence nothing to do */
    } else {
    ...
    Asc_DStringAppend(hptr,"Types are not conformable\n"
                     "or the Library is inconsistent", HALL);
    return HELP_ERROR;
  }
}
 ...
 */
int ascjson::Asc_BrowMakeAlikeCmd(Asc_DString *hptr,
                     int argc, CONST84 char *argv[])
{
  struct TypeDescription *desc,*desc1,*desc2;
  struct Instance *i1, *i2;
  ...
  if ( argc != 3 ) {
    Asc_DStringSet(hptr, "wrong # args : Usage \"bmakealike\" current search");
    return HELP_ERROR;
  }
  i1 = g_curinst;
  i2 = g_search_inst;
  if (i1 && i2) {
    ...
    if (desc1==desc2) {
      ...
      if (0 != (desc = MoreRefined(desc1,desc2))) {
        ...
        g_curinst = i1;
        g_search_inst = i2;		/* patch the pointers */
        Asc_DStringSet(hptr, "Instances are unconformable");
        return HELP_ERROR;
      }
    }
  } else {
    Asc_DStringSet(hptr, "Invalid instances in b_makealike");
    return HELP_ERROR;
  }
  return HELP_OK;
}
...
int ascjson::Asc_BrowAnonTypesCmd(Asc_DString *hptr,
                         int argc, CONST84 char **argv)
{
  struct Instance *i;
  if ( argc != 2 ) {
    Asc_DStringAppend3(hptr, "Usage: ",Asc_BrowAnonTypesCmdHN,
        " <-current,-search>", HALL);
    return HELP_ERROR;
  }
