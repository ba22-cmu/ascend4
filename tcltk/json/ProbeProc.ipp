...
/*
 * Adds the value of the entry passed to it to the hptr as an
 ...
 * functions it calls.
 */
int ascjson::AppendEntryItem(Asc_DString *hptr,struct ProbeEntry *e)
{
  char *name;
  ...
  if (e==NULL) {
    VTcl_AppendElement(hptr,"UNDEFINED probe entry");
    return 0;
  }
  ...
  Asc_DStringAppend(hptr,"{",HALL);	/* start element */
  Asc_DStringAppend(hptr,name,HALL);	/* append name */
  if (i==NULL) {
    Asc_DStringAppend(hptr," = UNCERTAIN} ",HALL);
    return 0;
  }
  switch(InstanceKind(i)) {
  case REL_INST:
  ...
  case REAL_CONSTANT_INST:
    ...
    if (ustr!=NULL) {
      Asc_DStringAppend2(hptr," = ",ustr,HALL);
    } else {
      Asc_DStringAppend2(hptr," = ","????",HALL);
    }
    Asc_DStringAppend(hptr,"} ",HALL);	/* end REAL */
    break;
  case INTEGER_INST:
  case INTEGER_ATOM_INST:
  case INTEGER_CONSTANT_INST:
  ...
  case LREL_INST:
    tmp[0] = '\0';
    Asc_DStringAppend2(hptr,tmp," = ",HALL);
    (void)Asc_BrowWriteAtomValue(tmp,i);
    Asc_DStringAppend2(hptr,tmp,"} ",HALL); /* end OTHERS */
    break;
  default:
    Asc_DStringAppend(hptr,"} ",HALL);	/* end default */
    break;
  }
  return 0;
}

void ascjson::ProbeAppendAll(Asc_DString *hptr, struct gl_list_t *p)
{
  unsigned long c,len;
  struct ProbeEntry *e;
  if (p != NULL) {
 ...
 */
static
void DefineProbeFilters(Asc_DString *hptr)
{
  VTcl_AppendElement(hptr,"VisitRelations/Collect relations");
  VTcl_AppendElement(hptr,"VisitLogRelations/Collect logical relations");
  VTcl_AppendElement(hptr,"VisitBooleans/Collect booleans");
  VTcl_AppendElement(hptr,"VisitIntegers/Collect integers");
  VTcl_AppendElement(hptr,"VisitReals/Collect reals");
  VTcl_AppendElement(hptr,"VisitSymbols/Collect symbols");
  VTcl_AppendElement(hptr,"VisitSets/Collect sets");
  VTcl_AppendElement(hptr,"VisitSABooleans/Collect subatomic booleans");
  VTcl_AppendElement(hptr,"VisitSAIntegers/Collect subatomic integers");
  VTcl_AppendElement(hptr,"VisitSAReals/Collect subatomic reals");
  VTcl_AppendElement(hptr,"VisitSASymbols/Collect subatomic symbols");
  VTcl_AppendElement(hptr,"VisitSASets/Collect subatomic sets");
  VTcl_AppendElement(hptr,"VisitBooleanConstants/Collect boolean constants");
...
void ProbeGarbageCollect(int number)
{
  ...
  for (c = 1; c <= len; c++) {
    ...
    if (ProbeEntryInst(e)!=NULL) {
      gl_append_ptr(lnew,e);
    } else {
      ProbeEntryDestroy(e);
    }
  }
  g_probe_array[number] = lnew;
  gl_destroy(old);
}
/*
 * macro ParseCollectionNumber assumes int status, char *argv[],
 * Asc_DString *hptr , int number, int argc.Forces a return with
 * an appropriately filled hptr if argv[2] is not an int or out of range.
...
#define ParseCollectionNumber(dummy) \
  ...
  status = JTcl_GetInt(hptr, argv[2],&number); \
  if (status != HELP_OK) { \
    Asc_DStringAppend6(hptr, "\nError in probe collection number ", \
      argv[0]," ", argv[1]," ", argv[2],HALL); \
    return HELP_ERROR; \
  } \
 ...
 * Asc_DString *hptr , int number, int index, int argc.Forces a return with
 * an appropriately filled hptr if argv[2] is not an int or out of range.
 * This ugly macro is only for use in Asc_ProbeCmd.
...
#define ParseCollectionIndex(number,ndx) \
  ...
  status = JTcl_GetInt(hptr, argv[(ndx)],&indexe); \
  if (status != HELP_OK) { \
    Asc_DStringAppend8(hptr, "\nError in collection index: ", \
      argv[0]," ", argv[1]," ", argv[2]," ",argv[(ndx)],HALL); \
    return HELP_ERROR; \
  } \
  if ( indexe < 0 || \
       (unsigned long)indexe >= gl_length(g_probe_array[(number)])) { \
    Asc_DStringFree(hptr); \
    Asc_DStringAppend7(hptr,Asc_ProbeCmdHN " ",argv[1]," ",argv[2]," ", \
      argv[(ndx)], " : index out of list range.", HALL); \
    return HELP_ERROR; \
  }
...
int ascjson::Asc_ProbeCmd(Asc_DString *hptr, int argc, CONST84 char *argv[])
{
  ...
  if (argc <2) {
    Asc_HelpGetUsageDS(hptr,Asc_ProbeCmdHN);
    return HELP_ERROR;
  };
  /*
  ...
  case 'a': /* add */
    if (argc<4) {
      Asc_DStringAppend3(hptr,"Not enough arguments to "
    ...
    if (argc ==4) {
      ...
      if (status!=0) {
        Asc_DStringAppend9(hptr, Asc_ProbeCmdHN " ",argv[1]," ",argv[2],
    ...
    if (argc == (4+NUMFILTERS)) {
      ...
      Asc_DStringAppend9(hptr,"Not enough arguments to " 
        Asc_ProbeCmdHN " ",argv[1]," ",argv[2]," ",argv[3]," [filter-list]: "
        "filter-list needs ",buf," boolean values.",HALL);
      return HELP_ERROR;
    }
    break;
  case 'c': /* clear */
    ...
    if (argc==3) { /* clear whole list */
      ...
      if (newlist==NULL) {
        Asc_DStringFree(hptr);
        Asc_DStringAppend5(hptr,Asc_ProbeCmdHN " ",argv[1]," ",argv[2],
          " : not enough memory",HALL);
        return HELP_ERROR;
      }
      pos = 3;
      c = 1;
      ...
      while(pos < argc) { /* copy list, except for the given indices. */
        ...
        if (indexe <= oldindexe) { /* no duplicates or reversals allowed */
          ...
          newlist = NULL;
          Asc_DStringFree(hptr);
          Asc_DStringAppend6(hptr,Asc_ProbeCmdHN " ",argv[1]," ",argv[2],
            " : index out of order ",argv[pos],HALL);
          return HELP_ERROR;
        }
        oldindexe = indexe;
  ...
  case 'e': /* expand */
    if (argc!=2) {
      Asc_DStringAppend2(hptr,"Too many arguments to " 
        Asc_ProbeCmdHN " ",argv[1],HALL);
      return HELP_ERROR;
    }
    if (Asc_ProbeArrayGrow()) {
      Asc_DStringAppend4(hptr,"Insufficient memory to ",
        argv[0]," ",argv[1],HALL);
      return HELP_ERROR;
    }
    ...
    break;
  case 'f': /* filter */
    DefineProbeFilters(hptr);
    break;
  case 'g': /* get */
    ParseCollectionNumber(0);
    if (argc==3) {
      /* get whole list */
      ProbeAppendAll(hptr,ProbeArray(number));
    } else {
      /* get the given indices. */
      pos = 3;
      while(pos < argc) {
        ParseCollectionIndex(number,pos);
        AppendEntryItem(hptr,ProbeGetEntry(number,indexe));
        pos++; /* next index */
      }
    }
    break;
  ...
  case 'n': /* name */
    if (argc!=4) {
      Asc_DStringAppend4(hptr,Asc_ProbeCmdHN," ",argv[1]," <number> <index>",
        HALL);
      return HELP_ERROR;
    }
    ParseCollectionNumber(0);
    ParseCollectionIndex(number,3);
    Asc_DStringAppend(hptr,ProbeEntryName(ProbeGetEntry(number,indexe)),
      HALL);
    break;
  case 'q': /* qlfdid */
    if (argc!=4) {
      Asc_DStringAppend3(hptr,Asc_ProbeCmdHN " ",argv[1]," <number> <index>",
        HALL);
      return HELP_ERROR;
    }
    ParseCollectionNumber(0);
    ...
    if (g_search_inst==NULL) {
      Asc_DStringAppend(hptr,"0",HALL);
    } else {
      Asc_DStringAppend(hptr,"1",HALL);
    }
    break;
  case 's': /* size */
    /*
